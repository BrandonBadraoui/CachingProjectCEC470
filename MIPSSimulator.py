...
BranchAddress = {0b000100: lambda Zero, greatherThanZero: (branch_target if (Zero) else PC_plus_4),  # beq
                 0b000001: lambda Zero, greatherThanZero: (
                     branch_target if ((greaterThanZero == 1) or (Zero == 1)) else PC_plus_4)}  # bgez
...
for clockcount in range(170):
    ...
    if (clock >= 3):
        ...
        alu_src2 = ID_EX[0][2] if ALUSrc[0] else ID_EX[0][
            1]  # if ALUsrc_current = 1 then sign Extend is alu_src2 else read_data2
        readData2 = ID_EX[0][1]
                ...
                if ((MemWrite[0] == 0) and (MemWrite[1] == 0) and (MemWrite[2] == 0)):  #
                    ...
                    if (WB_hazard_RD_check == my_rs[2]):  # with RS
                        if ((ALUOp[0] == 3)):  # if I instruction
                            ...
                            alu_src1_rs = MEM_WB[0][
                                1]  # load into alu_src1 the value of ALU result                        #
                            hazardDetected_rs = 1
                        else:
                            print("--------------------------RS forwaded from WB stage. RS = %d" % (MEM_WB[0][0]))  #
                            alu_src1_rs = MEM_WB[0][
                                0]  # load into alu_src1 the value of the loaded data from memory   #
                            hazardDetected_rs = 1
                    ...
                    if (MEM_hazard_RD_check == my_rs[2]):  # with RS
                        ...
                        alu_src1_rs = EX_MEM[0][
                            0]  # load into alu_src1 the value of ALU result                        #
                        hazardDetected_rs = 1

                    if ((MEM_hazard_RD_check == my_rt[2]) and (
                            ALUSrc[0] == 0)):  # with RT AND source is a reg not signext#
        ...
        if (stallDetected == 0):
            ...
            print("Zero = " + str(Zero) + " greaterThanZero = " + str(greaterThanZero) + "  Next_PC = " + str(
                PC) + "  Branch = " + str(Branch[0]))
